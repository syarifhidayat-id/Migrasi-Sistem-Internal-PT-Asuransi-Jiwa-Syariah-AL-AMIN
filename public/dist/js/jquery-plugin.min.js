...
function setTextReadOnly(kode, kondisi) {
    ...
    if (!input) {
        if (kondisi == true) {
            ...
            input.css({ 'background-color': 'rgba(255, 204, 138, 0.514)' });
...
function setTextBoxReadOnly(kode, kondisi) {
    const id = $('#' + kode);
    if (kondisi == true) {
        return id.textbox('readonly', kondisi);
    } else if (kondisi == false) {
        return id.textbox('readonly', kondisi);
    }
}
...
function comboGrid (kode, metod, url, idFild, textFild, colom, onselect) {
    $('#' + kode).combogrid({
        // delay: 500,
        mode:'remote',
        method: metod,
