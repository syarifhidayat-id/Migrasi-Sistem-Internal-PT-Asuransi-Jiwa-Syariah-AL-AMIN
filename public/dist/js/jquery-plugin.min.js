...
function selectSideTag(kode, url, data, getVal) {
    $('#' + kode).select2({
        tags: true,
        ajax: {
            url: url,
...
function onSelect (kode, uri, idVal, textVal) {
    var studentSelect = $('#' + kode);
    $.ajax({
        type: 'GET',
        url: uri
    }).then(function (data) {
        // create the option and append to Select2
        var option = new Option(data.textVal, data.id, true, true);
...
function selectSide(kode, kondisi, url, dataRes, getVal) {
    ...
    if (kondisi==true) {
        ...
        id.next().find('span.select2-selection').css({ 'background-color': '#ffc8c883' });
    }
    if (kondisi==false) {
        id.select2({
            ajax: {
        ...
        id.next().find('span.select2-selection').css({ 'background-color': '' });
    }
}
