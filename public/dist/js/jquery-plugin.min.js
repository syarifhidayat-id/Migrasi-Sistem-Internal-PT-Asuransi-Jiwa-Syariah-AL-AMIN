...
function selectServerSide(kode, url, data, getVal, kondisi) {
    const id = $('#' + kode);
    if (kondisi==true) {
        id.select2({
            ajax: {
        ...
        id.next().find('span.select2-selection').css({ 'background-color': '#ffcc8a83' });
    }
    if (kondisi==false) {
        id.select2({
            ajax: {
...
function setRequired (kode, kondisi) {
    const id = $('#' + kode);
    if (id.hasClass('form-control')) {
        if (kondisi == true) {
