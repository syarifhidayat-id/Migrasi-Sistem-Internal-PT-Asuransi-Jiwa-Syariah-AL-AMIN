...
function selectTable (kode, url, valText, valId, field, fieldcustom, getValue) {
    $("#" + kode).inputpicker({
        headShow: true,
        url: url,
        // urlHeaders: {
...
function setText(kode, value) {
    var input = $('input[id="'+kode+'"]');
    var select = $('#' + kode);
    if (!input) {
        input.val(value);
    } else {
        select.val(value);
        select.append(new Option("0", value));
    }
}
...
function setTextReadOnly(kode, kondisi) {
    ...
    if (!input) {
        if (kondisi == true) {
            input.prop('readonly', kondisi);
            input.addClass('required_sel');
        } else if (kondisi == false) {
            input.prop('readonly', kondisi);
            input.removeClass('required_sel');
        }
    } else {
        if (kondisi == true) {
            select.addClass('required_sel');
            select.prop('disabled', kondisi);
        } else if (kondisi == false) {
            select.removeClass('required_sel');
            select.prop('disabled', kondisi);
        }
    }
}
