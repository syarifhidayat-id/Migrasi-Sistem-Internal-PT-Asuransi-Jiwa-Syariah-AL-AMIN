...
function setTextReq(kode, kondisi) {
    ...
    if (id.hasClass('form-select')) {
        if (kondisi == true) {
            id.select2().next().find('[aria-labelledby="select2-'+kode+'-container"]').css({ 'background-color': '#ffc8c883' });
        } else if (kondisi == false) {
...
function jsonForm(id, respon) {
    ...
    if (hasClass('easyui-textbox') || hasClass('easyui-combobox') || hasClass('easyui-combogrid')) {
        $('#' + id).form('load', respon);
    }
}
