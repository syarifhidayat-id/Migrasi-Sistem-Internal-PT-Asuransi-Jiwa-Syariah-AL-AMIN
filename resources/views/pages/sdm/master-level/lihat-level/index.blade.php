    ...
    <div class="card-header border-0 pt-6">
        <div class="card-title">
            <div class="d-flex align-items-center position-relative my-1">
                ...
                <input type="text" data-kt-datatable-table-filter="search" class="form-control form-control-solid w-250px ps-14" placeholder="Cari level" />
            </div>
        </div>
                ...
                <div class="menu menu-sub menu-sub-dropdown col-xl-6" data-kt-menu="true">
                    ...
                    <div class="px-7 py-5" data-kt-datatable-table-filter="form">
                        <div class="row row-cols-1 row-cols-sm-2 rol-cols-md-1 row-cols-lg-2">
                            ...
                            <select class="form-select form-select-solid fw-bolder" data-kt-select2="true" data-placeholder="Pilih Level" data-allow-clear="true" data-kt-datatable-table-filter="nama-menu" data-hide-search="false">
                                <option></option>
                                @foreach ($carilevel as $key=>$data)
                                    <option value="{{ $data->slv_keterangan }}">{{ $data->slv_keterangan }}</option>
                                @endforeach
                            </select>
                                </div>
                            </div>
            ...
            <table class="table table-rounded table-striped border align-middle gy-5 gs-5" id="kt_table_datatable">
                <thead>
                    <tr class="fw-bold fs-6 text-gray-800 border-bottom border-gray-200 text-center align-middle" >
                        ...
                        <th>Uang Makan</th>
                        <th>Uang Saku</th>
                        <th>Uang Saku (sabtu/minggu)</th>
                        <th>Uang Saku Dinas Keluar Negeri</th>
                ...
                <tbody>
                    @foreach ($tampillevel as $key=>$data)
                        <tr>
                            <td class="text-center">{{ ++$key }}.</td>
                            <td>{{ $data->slv_kode }}</td>
                            <td>{{ $data->slv_keterangan }}</td>
                            <td>{{ $data->slv_tunjangan }}</td>
                            <td>{{ $data->slv_tun_inap }}</td>
                            <td>{{ $data->slv_tun_makan }}</td>
                            <td>{{ $data->slv_tun_saku_wd }}</td>
                            <td>{{ $data->slv_tun_saku_we }}</td>
                            <td>{{ $data->slv_tun_saku_lu }}</td>
                            <td>{{ $data->slv_tun_angkot }}</td>
